...
import 'package:gotravelclub/views/listado_cotizacion/card_cotizacion_vuelo.dart';
...
class CotizacionController extends GetxController {
  ...
  void getAllCotizacions() async {
    ...
    CotizacionResponse cotizacionResponse =
        await cotizacionService.getCotizacion();
    ...
    listInfo.forEach((element) {
      ...
      if (element.type != 4) {
        listadoCotizacionWidget.add(CardCotizacion(
            color: c,
            title: "${title} (${element.title})",
            icon: icon,
            info: element));
      } else if (element.type == 4) {
        listadoCotizacionWidget.add(CardCotizacionVuelo(
            color: c,
            title: "${title} (${element.clase})",
            icon: icon,
            info: element));
      }
    });
